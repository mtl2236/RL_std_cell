...
"""
import sys
sys.path.append("/simulation/tianliang/test/RL_std_cell/eval_engines/ngspice/")
from TwoStageClass import *

sys.path.append("/simulation/tianliang/test/RL_std_cell/eval_engines/util/")
from core import *
...
class TwoStageAmp(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        self.observation_space = spaces.Box(
            low=np.array(len(self.params_id)*[0]),
            high=np.array(len(self.params_id)*[100]))
    ...
    def reset(self):
        ...
        self.cur_params_idx = np.array([0, 0, 0, 9, 0, 0, 0])
        ...
        #self.ob = np.concatenate([np.array(reward), self.cur_params_idx])
        self.ob=self.cur_params_idx
        #print("this is ob!")
        #print(self.ob)
        return self.ob
...
#        self.cur_params_idx = self.cur_params_idx + np.array(self.action_arr[int(action)])
        self.cur_params_idx = np.clip(self.cur_params_idx, [1]*len(self.params_id), [(len(param_vec)-1) for param_vec in self.params])
    ...
    def update(self, params_idx):
        ...
        cur_specs=self.sim_env.create_design_and_simulate(param_val[0])
        return cur_specs
